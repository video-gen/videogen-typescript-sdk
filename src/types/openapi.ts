...
export interface webhooks {
  ...
  "project_export.succeeded": {
    ...
    /**
     * project_export.succeeded
     * @description Delivered when a project export started via the API completes successfully.
     */
    post: operations["projectExportSucceeded"];
    delete?: never;
    options?: never;
    head?: never;
    patch?: never;
  ...
  "project_export.failed": {
    ...
    /**
     * project_export.failed
     * @description Delivered when a project export started via the API fails.
     */
    post: operations["projectExportFailed"];
    delete?: never;
    options?: never;
    head?: never;
    patch?: never;
  ...
  "project_export.cancelled": {
    ...
    /**
     * project_export.cancelled
     * @description Delivered when a project export started via the API is cancelled.
     */
    post: operations["projectExportCancelled"];
    delete?: never;
    options?: never;
    head?: never;
    patch?: never;
...
export interface components {
  schemas: {
    ...
    WorkflowRunWebhookEventName:
      ...
      | "workflow_run.cancelled";
    /**
     * @description Lifecycle events emitted for project exports started via the developer API (`POST /v1/projects/{projectId}/export`).
     * @enum {string}
     */
    ProjectExportWebhookEventName:
      | "project_export.succeeded"
      | "project_export.failed"
      | "project_export.cancelled";
    /**
    ...
    RemixAction:
      ...
      | components["schemas"]["RemixActionAddZoom"]
    ...
    /** @description Body POSTed to a registered webhook endpoint when a project export started via the API reaches a terminal state. Use `exportId` with `GET /v1/projects/{projectId}/exports/{exportId}` for signed download URLs. */
    ProjectExportWebhookPayload: {
      event: components["schemas"]["ProjectExportWebhookEventName"];
      /** @description Opaque export id matching the original request (e.g. `vg_expo_...`). */
      exportId: string;
      /** @description Id of the exported project (e.g. `vg_proj_...`). */
      projectId: string;
      /** @description Seconds since epoch (Unix timestamp) at which VideoGen observed the terminal state. */
      occurredAt: number;
      /** @description File id (e.g. `vg_file_...`) of the rendered MP4. Always present as a field; `null` until `project_export.succeeded`. Pass it to `POST /v1/files/{fileId}/hydrate` for signed download URLs. */
      exportFileId: string | null;
      /** @description Error details. Present (non-null) only on `project_export.failed`; `null` otherwise. */
      error?: components["schemas"]["ApiError"] | null;
    };
...
export interface operations {
  ...
  projectExportSucceeded: {
    ...
    requestBody: {
      content: {
        "application/json": components["schemas"]["ProjectExportWebhookPayload"];
      };
    };
    responses: {
      /** @description Acknowledge receipt */
